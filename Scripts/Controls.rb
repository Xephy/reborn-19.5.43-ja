class DefaultKeyboardControlsScene
  def pbStartScene
    ...
    # The control list is built from bare strings, so the compiler never
    # registered a key for any of it. Look each line up here, before it is drawn
    # and before @texts is kept for the screen reader. A line with no
    # translation comes back unchanged, so English is untouched.
    texts.each { |t| t[0] = _INTL(t[0]) }
    @texts = texts
    @index = 0
    tts(texts[0][0])
