...
# `lineheight` lets a caller pack more lines into the same pixel budget. It
# defaults to the hard-coded 32 the rest of the game expects, so existing
...
def getLineBrokenChunks(bitmap, value, width, dims, plain = false, lineheight = 32)
  ...
  while (c = textmsg.slice!(/\n|\S*\-+|(\S*([ \r\t\f]?))/)) != nil
    ...
    if ccheck == "\n"
      x = 0
      y += (textheight == 0) ? lineheight : textheight
      textheight = 0
      next
    end
    if ccheck[/</] && !plain
...
def drawTextExFitted(bitmap, x, y, width, numlines, text, baseColor, shadowColor, minsize = 20)
  ...
  loop do
    ...
    # Line height follows the font, so a smaller size buys extra LINES inside
    # the same pixel budget rather than just narrower glyphs. That is what lets
    # a long description stay complete instead of being clipped: the ability
    # slot is 282px x 64px, which holds two lines at full size but three once
    # the font steps down.
    lh = bitmap.text_size("A").height
    lh = 1 if lh < 1
    dims = [0, 0]
    normtext = getLineBrokenChunks(bitmap, text, width, dims, true, lh)
    break if dims[1] <= maxheight || size <= minsize
