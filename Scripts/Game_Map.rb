class Game_Map
  ...
  def autoplay(from_startup = false)
    return if Reborn && [826, 827].include?($game_map.map_id) && !from_startup
  ...
  def name
    # Same lookup as everywhere else, so the signpost shown on entering an area
    # cannot disagree with the name on the save screen. It reads the map-name
    # message section and expands \PN just as this did.
    return pbGetMapNameFromId(self.map_id)
  end
