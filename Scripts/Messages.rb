...
class Interpreter # Used by RMXP
  ...
  def command_102
    ...
    cmdlist = []
    @list[@index].parameters[0].each do |command|
      cmdlist.push(_MAPINTL($game_map.map_id, command))
    end
    command = Kernel.pbShowCommands(nil, cmdlist, @list[@index].parameters[1])
    @message_waiting = false
    @branch[@list[@index].indent] = command
...
# Dozens of call sites hand the message functions an English literal instead of
# running it through _INTL first. Looking the text up here catches all of them
# without touching every call site, and costs one hash probe per message: text
# that has already been translated is not a key in the table, so it comes back
# unchanged. Only whole strings can be caught this way — anything built by
# interpolation has to be rewritten at the call site.
def Kernel.pbLocalize(text)
...
def Kernel.pbMessage(message, commands = nil, cmdIfCancel = 0, skin = nil, defaultCmd = 0, tts: true, &block)
  message = Kernel.pbLocalize(message)
  commands = commands.map { |c| Kernel.pbLocalize(c) } if commands.is_a?(Array)
  ret = 0
...
def Kernel.pbChoice(message, commands)
  keys = commands.keys
  values = commands.values
  choice = Kernel.pbMessage(message, values, -1)
...
def Kernel.pbMessageDisplay(msgwindow, message, letterbyletter = true, commandProc = nil, tts: true)
  ...
  if Rejuv
    ...
    text.gsub!(/\\gsc/i, "<fn=PKMN RBYGSC><fs=22>")
    text.gsub!(/\\egsc/i, "</fs></fn>")
  end
  text.gsub!(/\\\\/, "\5")
  if $game_actors
    text.gsub!(/\\n\[([1-8])\]/i) {
  ...
  text.gsub!(/\\pm/i, _INTL("${1}", pbCommaNumber($Trainer.money))) if $Trainer
  text.gsub!(/\\n/i, "\n")
  text.gsub!(/\\\[([0-9A-Fa-f]{8,8})\]/) { "<c2=" + $1 + ">" }
  text.gsub!(/\\pg/i, "\\b") if $Trainer && $Trainer.isMale?
  text.gsub!(/\\pg/i, "\\r") if $Trainer && $Trainer.isFemale?
  text.gsub!(/\\pog/i, "\\r") if $Trainer && $Trainer.isMale?
  text.gsub!(/\\pog/i, "\\b") if $Trainer && $Trainer.isFemale?
  text.gsub!(/\\pg/i, "")
  text.gsub!(/\\pog/i, "")
  text.gsub!(/\\b/i, "<c2=6546675A>")
  text.gsub!(/\\r/i, "<c2=043C675A>")
  text.gsub!(/\\1/, "\1")
  colortag = ""
  ...
  begin
    ...
    text.gsub!(/\\vu\[([0-9]+)\]/i) { $game_variables[$1.to_i].upcase }
  ...
  textchunks = []
  controls = []
  while text[/(?:\\(w|f|ff|ffr|ts|st|cl|me|se|wt|wtnp|ch|tts)\[([^\]]*)\]|\\(g|cn|ap|wd|wm|op|sh|cl|wu|\.|\||!|\^)())/i]
  ...
  ttsText = toUnformattedText(text)
  signWaitCount = 0
  shout = false
  ...
  for i in 0...controls.length
    ...
    if control == "f"
      ...
      ttsText = param
      ttsText.gsub!(/\\pn/i, $Trainer.name) if $Trainer
    end
  end
  tts(ttsText) if tts
  if startSE != nil
  ...
  if $Settings.dynamicTextbox == 1
    linecount = msgwindow.calculateLines(text).clamp(2, 4)
  end
