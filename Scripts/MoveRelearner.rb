...
class MoveRelearnerScene
  ...
  def pbStartScene(pokemon, moves)
    ...
    @moves = moves
    moveCommands = []
    moves.each { |i| moveCommands.push(getMoveShortName(i)) }
    # Create sprite hash
  ...
  def pbDrawMoveList
    ...
    # Every badge on this screen is blitted straight from AnimatedBitmap, which
    # takes the path as given and so never reaches Graphics/Icons/ja/.
    ...
    for i in 0...VISIBLEMOVES
      ...
      if moveobject
        ...
        if movedata
          typeimage = AnimatedBitmap.new(pbBitmapName(sprintf("Graphics/Icons/type%s", movedata.type)))
          overlay.blt(12, yPos + 2, typeimage.bitmap, type1rect)
          textpos.push([getMoveShortName(moveobject), 80, yPos, 0, Color.new(248, 248, 248), Color.new(0, 0, 0)])
          if movedata.maxpp > 0
...
class MoveRelearnerScreen
  ...
  def pbStartScreen(pokemon)
    ...
    unless pbHasRelearnableMove?(pokemon)
      Kernel.pbMessage(_INTL("There are no moves this Pokémon can relearn."))
      return false
    end
