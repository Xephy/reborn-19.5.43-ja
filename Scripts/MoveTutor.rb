  ...
  def pbDrawMoveList
    ...
    for i in 0...VISIBLEMOVES
      ...
      if moveobject
        ...
        if movedata
          # AnimatedBitmap takes the path as given, so the badge has to be
          # pointed at Graphics/Icons/ja/ explicitly.
    ...
    loop do
      ...
      if !move.is_a?(Symbol)
        ...
        if @scene.pbConfirm(_INTL("Teach {1}?", getMoveName(move)))
          @scene.pbDrawMoveList if pbMoveTutorChoose(move, false, false, true)
        end
      end
    end
  end
