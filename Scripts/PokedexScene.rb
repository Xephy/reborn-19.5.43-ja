...
=end
  ...
  def pbSearchDexList(params)
    ...
    if params[2] != 0 || params[3] != 0 # Filter by type
      ...
      for i in $cache.types.keys
        typeCommands.push(i) if ![:QMARKS, :SHADOW].include?(i)
      end
  ...
  def pbChangeToDexEntry(species)
    ...
    if $Trainer.pokedex.dexList[species][:owned?]
      mondata = $game_switches[:Randomized_Challenge] && ($Randomizer.randomTypes) ? $rndcache.pkmn[species, form] : $cache.pkmn[species, form]
      type1 = mondata.Type1
      type2 = mondata.Type2
      height = mondata.Height
      weight = mondata.Weight
      kind = getMonKind(species, form)
      dexentry = getMonDexEntry(species, form)
      ...
      # The dex type badges are 96x32 images with the type name drawn on them.
      # pbBitmapName picks up the Japanese copies under Pokedex/ja/.
      type1bitmap = AnimatedBitmap.new(pbBitmapName("Graphics/Pictures/Pokedex/pokedex#{type1.to_s}"))
      @sprites["overlay"].bitmap.blt(296, 118, type1bitmap.bitmap, Rect.new(0, 0, 96, 32))
      type1bitmap.dispose

      if type2 != nil
        type2bitmap = AnimatedBitmap.new(pbBitmapName("Graphics/Pictures/Pokedex/pokedex#{type2.to_s}"))
    ...
    if $Trainer.pokedex.dexList[species][:owned?]
      tts(_ISPRINTF("{1:s} Pokémon", kind))
      if type2 != nil
