...
class PurifyChamberScreen
  ...
  def pbOnPlace(pkmn)
    ...
    if @chamber.setCount(set) == 0 && @chamber.isPurifiableIgnoreRegular?(set)
      ...
      @scene.pbDisplay(
        _INTL(
          "This {1} is ready to open its heart. However, there must be at least one regular Pokémon in the set to perform a purification ceremony.", pkmn.name
        )
      )
    end
  end
      ...
      if cmd[0] == 0
        ...
        if curpkmn || heldpkmn
          ...
          choice = @scene.pbShowCommands(
            _INTL("What shall I do with this {1}?", heldpkmn ? heldpkmn.name : curpkmn.name), commands
          )
          if choice == 0
            if heldpkmn
...
class Window_PurifyChamberSets < Window_DrawableCommand
  ...
  def drawItem(index, count, rect)
    ...
    if index == @switching
      textpos.push([_ISPRINTF("{1:d}", index + 1), rect.x, rect.y, false, Color.new(248, 0, 0), self.shadowColor])
    else
      textpos.push([_ISPRINTF("{1:d}", index + 1), rect.x, rect.y, false, self.baseColor, self.shadowColor])
    end
    if @chamber.setCount(index) > 0
      pbDrawGauge(
        self.contents, Rect.new(rect.x + 16, rect.y + 6, 48, 8),
        Color.new(0, 0, 256), @chamber[index].tempo, PurifyChamber.maximumTempo()
      )
    end
    if @chamber.getShadow(index)
      pbDrawGauge(
        self.contents, Rect.new(rect.x + 16, rect.y + 18, 48, 8),
...
class PurifyChamberSetView < SpriteWrapper
  ...
  def refresh
    ...
    textpos = []
    if pkmn
      if pkmn.type2.nil? || pkmn.type1 == pkmn.type2
        textpos.push(
          [_INTL("{1}  Lv{2}  {3}", pkmn.name, pkmn.level, getTypeName(pkmn.type1)), 2, 0, 0, Color.new(248, 248, 248), Color.new(128, 128, 128)]
        )
      else
        textpos.push(
          [_INTL("{1}  Lv{2}  {3}/{4}", pkmn.name, pkmn.level, getTypeName(pkmn.type1), getTypeName(pkmn.type2)), 2, 0, 0, Color.new(248, 248, 248), Color.new(128, 128, 128)]
        )
      end
      textpos.push([_INTL("FLOW"), 2 + @info.bitmap.width / 2, 24, 0, Color.new(248, 248, 248), Color.new(128, 128, 128)])
      # draw heart gauge
      pbDrawGauge(
        @info.bitmap, Rect.new(@info.bitmap.width * 3 / 4, 8, @info.bitmap.width * 1 / 4, 8),
        Color.new(192, 0, 256), pkmn.heartgauge,
        PokeBattle_Pokemon::HEARTGAUGESIZE
      )
      # draw flow gauge
      pbDrawGauge(
        @info.bitmap, Rect.new(@info.bitmap.width * 3 / 4, 24 + 8, @info.bitmap.width * 1 / 4, 8),
        Color.new(0, 0, 248), @chamber.chamberFlow(@set), 6
      )
    end
    if @chamber.setCount(@set) > 0
      textpos.push([_INTL("TEMPO"), 2, 24, 0, Color.new(248, 248, 248), Color.new(128, 128, 128)])
      # draw tempo gauge
      pbDrawGauge(
        @info.bitmap, Rect.new(@info.bitmap.width * 1 / 4, 24 + 8, @info.bitmap.width * 1 / 4, 8),
...
class PurifyChamberScene
  ...
  def pbOpenSet(set)
    ...
    @viewportmsg.z = 99999
    addBackgroundOrColoredPlane(@sprites, "bg", "purifychamberbg", Color.new(0, 0, 0), @viewport)
