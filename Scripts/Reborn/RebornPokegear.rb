...
class Scene_FieldNotes
  def buildFieldMenu
    ...
    menu = []
    general = {
      # This class reopens the one in FieldNotes.rb and replaces the whole menu,
      # so the field names have to be looked up here too - the colour tags are
      # kept out of the translated part.
      label: "<c3=f8c471,8a461e>" + _INTL("General"),
      field: :INDOOR,
    }
    menu.push(general)
    FIELD_NOTES_MENU.each do |category, fields|
      ...
      fields.each do |field|
        if seenFields.include?(field)
          item = {
            label: "      <c3=f8f8f8,5c5c5c>" + fieldNoteName(field),
            field: field,
          }
        else
          item = {
    ...
    back = {
      label: "<c3=a7a7a7,4e4e4e>" + _INTL("Back"),
      back: true,
    }
    menu.push(back)
    menu
