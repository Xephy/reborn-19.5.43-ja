...
def pbShowThemeTeams(partnerPickin, total_trainers_needed = 1, randomized = false)
  ...
  sprites["arrow"].y = 48
  sprites["arrow"].z = 102
  tts(trainernames[sprites["index"]])
  loop do
    Graphics.update
    Input.update
...
def themeTeamArray
  teams = [
    ...
    { number: 16, trainer: "Amaria", name: "Boss Rush", teamnumber: 10, field: :WATERSURFACE, doubles: $game_switches[881] },
    { number: 16, trainer: "Amaria", name: "Boss Rush 2", teamnumber: 11, field: :UNDERWATER, doubles: $game_switches[881] },
...
def chooseThemeTeams(randomized)
  ...
  if $game_switches[1400]
    Kernel.pbMessage(_INTL("What team should {1} use?", $game_variables[28]))
  else
    Kernel.pbMessage(_INTL("Which one of {1}'s teams would you like to face?", $game_variables[28]))
  end
...
# Includes :FLOWERGARDEN1 but not the higher levels.
def pbFieldSymbols
  return FieldIDToSym.values
end

def pbRandomField
  return pbFieldSymbols.sample
end

def pbChooseField(message = nil, roll_random: true)
  message = _INTL("What field would you like to battle on?") if message.nil?
  fields = {}
  fields[:INDOOR] = _INTL("No Field")
  fields[:RANDOM] = _INTL("Random")
  fieldsyms = pbFieldSymbols
  fieldsyms.sort_by! { |field| getFieldName(field) }
  fieldsyms.each do |field|
    fields[field] = getFieldName(field)
  end
  choose = Kernel.pbChoice(message, fields)
  choose = fieldsyms.sample if roll_random && choose == :RANDOM
  choose = :INDOOR if choose.nil?
  return choose
end
