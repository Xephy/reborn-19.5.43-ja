  ...
  def pbItem(selected, heldpoke)
    ...
    if !pokemon.item.nil?
      ...
      if !item.nil?
        ...
        if itemname == "Blue Orb" && pokemon.species != :KYOGRE
          pbDisplay(_INTL("The Blue Orb can't be held!"))
          return
        end
        if itemname == "Red Orb" && pokemon.species != :GROUDON
          pbDisplay(_INTL("The Red Orb can't be held!"))
          return
        end
        if pbIsMail?(item)
          pbDisplay(_INTL("Can't attach a mail from PC."))
          return
        end
      ...
      when cmdSortPC # Sort PC
        ...
        if ret == -2
          if minbox == maxbox
            ...
            pbDisplay(_INTL("{1} to {2} is empty.", $PokemonStorage[minbox].name, $PokemonStorage[maxbox].name))
          end
        elsif ret == -1
        else
          @scene.pbHardRefresh
          if minbox == maxbox
            ...
            pbDisplay(_INTL("{1} to {2} was sorted.", $PokemonStorage[minbox].name, $PokemonStorage[maxbox].name))
          end
        end
    end
  end
