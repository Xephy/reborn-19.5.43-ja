...
def Kernel.pbMessageFreeText(message, currenttext, passwordbox, maxlength, width = 240, past_texts = [], &block)
  # The no-keyboard branch below already translates the prompt; the keyboard one
  # went straight to the message window with whatever it was handed.
  ...
  unless $Settings.useKeyboard?
    ...
    return screen.pbStartScreen(_INTL(message), 0, maxlength, currenttext)
  end
...
class PokemonEntryScene
  ...
  def pbEntry
    return $Settings.useKeyboard? ? pbEntry1 : pbEntry2
  end
