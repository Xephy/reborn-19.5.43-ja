...
def pbSetUpSystem
  ...
  if LANGUAGES.length >= 2
    # NOTE (JP localization): the stock code here read
    #     if !havedata
    #       $Settings.language = pbChooseLanguage
    #     end
    # but `havedata` is not defined anywhere in the project, so as soon as
    # LANGUAGES has 2+ entries this raised NameError during startup (called
    # from ClientData.rb#startup and from Load.rb). The language is instead
    # chosen from the title screen's "Language" command (Scripts/Load.rb),
    # which is only shown when LANGUAGES.length >= 2.
